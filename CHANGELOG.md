...
- **`testing.md`** — the `toHavePath` completion-signal discussion now covers a second failure mode: asserting on a *transient redirect hop*. In a client-side redirect chain (e.g. `/` → dashboard → `401` → `/sign-in`), a mid-chain URL exists for only one fetch round-trip, so `toHavePath` on it races — passing on a fast machine and failing on a loaded CI runner. Teaches asserting the final resting state instead (sign in first to make the target final; pin the unauthenticated end state separately), and notes that a spec that only fails on CI is usually racy in this way rather than newly broken. Also adds, to the inline-job "works in isolation, flaky in the suite" note, that single-file-green / full-suite-red is the default shape of a flake in this stack, so a flake fix must be validated against the full suite or the failing ordering, not the single file it was iterated in.
- **`querying.md`** — the `nestedSelect` section now states the two subquery shapes it cannot express — **correlated** (references an outer row) and **aggregate** (`count`/`sum`/… as the value) — and shows the fallback: drop to Kysely but source the inner table from a scoped Dream query (`AssocModel.query().toKysely('select')`), not raw `db().selectFrom('table')`, so the association's default scopes survive, correlating with `sql.ref('outer.col')` on the `whereRef` RHS.
