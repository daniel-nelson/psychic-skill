...
- **`models.md`** — the Transactions section names the cost that survives binding every write correctly: a sortable write holds the lock on its sort scope until the transaction commits, so a transaction writing `Room`s across many `Place`s holds one lock per place at once, and nothing releases them along the way. Advisory locks come from a table the whole Postgres cluster shares, so exhausting it fails other connections' transactions with `out of shared memory`, away from the code that caused it. The remedy is to narrow the transaction to the writes that must land together — a loop whose sortable writes span many `Place` scopes is not one of them — at the cost of atomicity across the loop, which leaves a re-run after a later iteration fails responsible for skipping what already committed. It also says never to unbind a write inside a transaction it did not open; the paragraph above it covers that opposite mistake, a write left unbound, whose fix is the reverse.
...
- **`SKILL.md`** — the Critical Rules are renumbered to close the gaps left at 11 and 20 when two rules were removed: 1-21, contiguous. The by-number cross-references in `testing.md`, `sti.md`, `workers.md`, `openapi.md`, `serializers.md` and `controllers.md` move with them. `CHANGELOG.md` keeps the numbers each entry was released under.
- **`SKILL.md`** — the Reference Map moves above the Critical Rules. Truncation keeps the start of a skill body, and when this file last exceeded the per-skill cap the entire routing map was dropped while the rules survived. The map is the half that makes the other seventeen files reachable, so it now sits where a cut reaches it last. Token-neutral, and no heading or anchor moves.
...
- **`SKILL.md`** — Critical Rule 23: reach for the simplest shape that satisfies the requirement, and escalate only on evidence you can point at. Before choosing something heavier than a plain `update`, a `findEach`, or a database constraint, name the concrete scenario that breaks the simple shape — a specific concurrent writer, a measured row count, an invariant the database cannot express — because "a race is conceivable" is not one and the heavier shape is not free. A one-shot data correction is named as the case that goes wrong most often: it is a `findEach` calling `update` on each affected row, or the `Query#update` callback form when the new value derives from the row, not a table lock or a manifest or a whole-population re-validation. A review finding that is an artifact of self-inflicted complexity is answered by removing the complexity, and the tell is a fix that adds a second mechanism to contain the first.
- **`locking.md`** — "A table lock is not the next step up". `lock` on a query is Dream's whole locking surface and it locks rows; a `LOCK TABLE` statement issued through raw `sql` blocks writers to every row in the table, including rows the work never touches, for the length of the transaction. A guard on specific records is `{ lock: true }`, an invariant the application cannot enforce alone is a database constraint, and a one-shot correction over a known set of rows is a `findEach`. The unbounded wait a table lock introduces is its own second problem, and needing `lock_timeout` to solve it is the signal to go back to the simple shape.
- **`SKILL.md`** — a truncation-recovery line above the H1. Compaction keeps the head of a skill and drops the tail, so a cut-off copy still has its first line; that line now names the file's last heading as a tripwire — if `## Troubleshooting Migrations` is missing, the copy is incomplete and the agent re-reads `SKILL.md` in full before acting on it. A re-read is an ordinary file read and is not subject to the per-skill cap, so recovery is complete. The file fits under the cap today, so the line should never fire — but the 25,000-token budget is shared across every invoked skill, and a session that invokes several after this one can lose the body whole.
- **`SKILL.md`** — a Reference Map in place of the 21 topic sections: one entry per reference file saying when to open it and what it owns. `querying.md`, `i18n.md`, `console.md` and `utils.md` gain the contents line the other thirteen already had. Project Structure lists `controllers/Admin`, `controllers/Internal` and `controllers/helpers`, which the scaffold creates.
- **`controllers.md`** — "Declaring Routes": `r.namespace`, `r.resources`, `r.resource`, nested resources, `r.collection` and simple routes, sited between the controller hierarchy the route file mirrors and `ApplicationController`. The worked route file shows an auth-loosening surface as its own top-level namespace with the version nested inside it. The file also now carries the two rationalizations for a catch-all `try`/`catch` — "I'm logging, not swallowing" and "it's only a small per-iteration catch" — beside the error-mapping table that answers them.
- **`openapi.md`** — why OpenAPI stays attached to serializers rather than hand-written `responses` blocks: it gives TypeScript one implementation surface for both the returned data and the documented schema, instead of letting a plain object and a duplicated schema drift independently.
- **`sti.md`** — generation order is migration order. When a child declares columns, `g:sti-child` writes a timestamp-named migration that `ALTER`s the parent's table, so a child generated ahead of its parent sorts ahead of the `CREATE TABLE` it depends on. A child with no columns generates no migration.
- **`websockets.md`** — the one case where `transports: ['websocket']` is more than a latency preference: engine.io's heartbeat timers are per-connection, so a multi-node fleet serving polling clients needs sticky sessions, and websocket-only transport sidesteps that.
- **`testing.md`** — `context` is **not** a Vitest global. Vitest ships `describe` and `it` only; Dream's spec helpers manufacture `context` (`global.context = describe`) so Psychic specs read RSpec-style, which is also where the `.staticMethod` / `#instanceMethod` describe names come from. Use it freely; nothing needs importing.
- **`MAINTAINING.md`** — a new maintainer document, listed in `README.md` outside the skill-files table because it is not skill content and is never linked from `SKILL.md`. It carries where the skill-authoring standard now lives and how to fetch it as raw markdown, why the binding size limit is 5,000 tokens rather than 500 lines, the per-rule calibration test with its quotations scoped to the sections they come from, the maintainer's rulings in his own words, the verification commands for a repo with no CI, and which validator failures are deliberate and must not be "fixed". It opens with a `Last verified` stamp naming what was checked against what, so refreshing it is a defined operation. `CLAUDE.md` points at it and repeats the four facts that bind whether or not anyone opens it.

### Changed

- **`workers.md`** — the debounce paragraph is written around what debounce is for: it guarantees the job runs at least once, at or after the moment it was last scheduled, and it collapses repeated expensive work when events fire in quick succession — it does not guarantee the work happens only once. The delay must be at least three seconds; a shorter one throws. Re-arming the same `jobId` from inside the job's own running handler is stated as safe on its own, without a justification. For work that must happen only once, the section says to record that it happened — a boolean or a `DateTime` column on the model — and return early when a later run finds it set, which makes a run that fires while a new event re-arms the timer a non-issue; when the timing rather than the collapsing is what matters, the shape is a scheduled job with a datetime check on the model.
- **`workers.md`** — "Rate Limiting" is what the section is called and what it documents: a named workstream's `rateLimit` bounds how many of its jobs start per time window, with no BullMQ Pro licence involved. The Overview lists queue-based rate limiting alongside retries, priorities, and scheduled jobs without qualification. Passing `QueuePro` and `WorkerPro` as the `Queue` and `Worker` providers is stated where Pro is actually the requirement — beside group priority.
- **`workers.md`** — workstream isolation and priority ordering hold together: the mapped priority is written to BullMQ's top-level `priority` on every job, grouped or not, so a per-call or class-level `priority` is honored on a service carrying a `workstream`, and a fan-out isolated onto its own named workstream keeps the expander/individual backpressure. A workstream's `name` becomes the `group.id` of its jobs and workers, a BullMQ Pro surface open-source BullMQ stores and ignores; Pro's `group.priority` sits alongside the job priority and orders a job's group against the queue's other groups.
...
- **`SKILL.md`** — restructured to fit the window that survives compaction. Claude Code re-attaches only the first 5,000 tokens of a skill body after auto-compaction and truncates from the end; the file was 10,974 tokens, so the cut fell at line 130 and took every pointer to all 17 reference files with it — past the first compaction an agent kept the Critical Rules and lost the map telling it which file answers which question. It is now 4,931 tokens across 124 lines, with nothing below the cut. Every Critical Rule keeps its statement; only justification moved, into the reference file that rule already pointed at — Rule 14's two rationalizations to `controllers.md` beside the error-mapping table they turn on, Rule 23's lock costs and its "name the concrete scenario" guard to `locking.md`, Rule 21's rationale to `openapi.md` — and the routing sample to `controllers.md`. Rule 15's `_never` switch and `Record` map stayed, being the mechanism rather than an explanation of it.
- **`SKILL.md`** — the frontmatter description triggers on how people actually ask. Every trigger was a code-state condition — an import path, a Dream model, a `psy` command — so a developer who types "why isn't this field in the response" matched none of them and the serializer guidance that answers it never loaded. The description now names five plain-language asks alongside the import paths and says the skill applies even when Dream, Psychic and `psy` are never mentioned. `@rvoh/psychic-spec-helpers` and the `@rvoh/dream/*` sub-paths join the triggers, and the Covers list gains i18n, the Dream console and deploying, each of which has a reference file it was not advertising.
- **`SKILL.md`** — the Critical Rules are classified by fragility rather than uniformly emphatic. Rules governing mechanically inevitable failures keep the rigid `ALWAYS`/`NEVER` form; rules 3, 5 and 6 were all-caps with no stated reason and each gains a mechanical one, relocated from where it already lived; rules 1, 13 and 18 trade the directive for the reasoning, and 13 and 18 lose rationales an agent could invert to opt out.
- **`SKILL.md`** — the `create-psychic` example no longer claims every option can be supplied on the command line. `create-psychic` prompts for `--runtime`, which the example omitted, and GitHub Actions has no flag at all, so the documented command hung on a select prompt in a non-TTY shell. Verified against `@rvoh/create-psychic` 3.7.8.
- **`SKILL.md`** — five statements the skill made two ways now agree with their reference file. The initializer rule stops implying work the scaffold already does and names the three cases that actually lose it; background arguments are "IDs and non-model scalars" rather than "IDs only", which had banned passing a page size; the preload rule states its real failure, `NonLoadedAssociation` on nested dependencies, instead of reading as a global ban; and `openapi.md` documents the `validate` `all` shorthand as overriding in both directions, which is what makes `validate: { all: false }` work as a debugging lever.
- **`generators.md`** — the aliased-FK `g:migration` example produces the `canceled_by_id` column and its index; a standalone `g:migration` adds no association, so the `canceledById` property and the `canceledBy` declaration are hand-added. The file had been claiming the association came free while the next bullet said otherwise.
- **Terminology across the skill** — one word per concept: "column" where the surrounding text already says column, and "spec" where Psychic's own vocabulary is spec (`pnpm uspec`, `pnpm fspec`, the `spec/` directory). Two sites on the `@deco.Encrypted` surface went to "property" instead, because both passages exist to say the decorated thing and the persisted column are different. The test database, `NODE_ENV=test`, smoke tests, "test" as a verb and "test" meaning a criterion are untouched.
- **`psychic-update-skill/SKILL.md`** — the description says when to reach for the skill. It names its three trigger classes — the psychic-skill preamble's update check reporting `UPGRADE_AVAILABLE`, the `/psychic-update-skill` invocation, and someone simply saying the skill is out of date or asking which version they are on — and closes by marking the boundary against its strongest false match, a request to update a project's `@rvoh/*` packages. `AskUserQuestion` joins its allowed tools, which the skill had always instructed the agent to use without permitting it.
- **`CLAUDE.md`** — a "Skill-authoring craft" section pointing at `MAINTAINING.md` and stating the four facts that bind regardless: the 5,000-token ceiling and today's headroom, where the standard lives, that a quotation's enclosing section is verified and not merely its text, and that three validators fail this repo on purpose.
...
- **`SKILL.md`** — Critical Rule 11 (comments explain why rather than what) and Critical Rule 20 (optional-parameter defaults belong in the signature). The ecosystem holds neither: Dream's `Query` constructor re-derives about twenty options in its body, `DateTime` uses the endorsed form at one method and the rejected one at another, and no lint rule enforces either — dream's entire eslint rules block is `no-console`, and psychic and create-psychic declare none. The numbering keeps gaps at 11 and 20 until the renumber, which happens once against the full set of by-number references.
- **`models.md`** — the instance-level `someModel.transaction(...)` branch, its example, and the class/instance framing around `Model.txn(null)`. `transaction` is declared `public static async` on `Dream` and nowhere else, so the instance call is a type error or `undefined` — an agent following it wrote code that could not run. The instance-side surface is `txn`, which the file documents correctly elsewhere.
- **`SKILL.md`** — the claim that Socket.IO's long-polling default causes "subtle stale-data failures". There were none. The failure it described was a crash — a health-check `request` listener writing headers to an already-sent polling response — fixed server-side in `@rvoh/psychic-websockets` 3.5.0, leaving connection latency as the live reason to set websocket-only transport. `websockets.md` had said so all along.
- **`SKILL.md`** — two Critical Rules that a later section of the same always-loaded file restated in full. The rules keep their statements; the restatements go.
- **Roughly 930 words of advice a capable model produces unprompted**, across nine files. The largest cuts: replica-lag figures and the model-namespacing generalities in `models.md`; four of the nine testing principles and the visual feature-spec debugging walkthrough in `testing.md`; the build-output notes and the deployed-service debugging checklist in `deploying.md`; the Kysely rule-of-thumb menu and the mixed SQL/Dream loading recipe in `querying.md`; the soft-delete use-case list in `soft-delete.md`; the fan-out problem list in `workers.md`; and the `jsonb` relational-alternative paragraph in `migrations.md`. Passages that defend a Psychic opinion against a prevalent practice are kept in full, and two of them now say what they are arguing against — spec organization states that `context` is manufactured rather than a Vitest global, and the BDD section names the ship-the-fix-first habit it rejects.
- **`psychic-update-skill/SKILL.md`** — the sentence narrating what the current upgrade script replaced.

## 0.88.0 — 2026-09-11

### Added

- **`workers.md`** — a "Key rules" entry: every dispatch appends the BullMQ `Job` as a final argument — `this.background(...)` and `this.backgroundWith(...)` pass only the arguments before it — and no parameter may carry a default or a `?`. Give the last parameter either and the appended `Job` lands in that slot, so the default never applies and the method runs with a `Job` where it expected its own value — in specs too, which dispatch through the same path. Scheduled methods are dispatched the same way but must not declare the `Job`: `schedule()` requires every parameter the method declares, so a final `job: Job` becomes an argument the call site has to pass.

### Changed

- **`workers.md`** — the "App-Owned Retry Budgets" geocoding example seeds its attempt count at the enqueue site, `this.background('_geocodePlace', place.id, 1)`, and `_geocodePlace` declares `attempt` as required; the prose describes a required argument the public entry method seeds.
- **`workers.md`** — "Backgrounded methods run inline in tests" covers `background(...)` and `backgroundWith(...)`; `schedule(...)` registers a BullMQ job scheduler in test as in every other environment, so a spec that calls it runs nothing.
- **`workers.md`** — "Job Logging" owns how a method receives the `Job`; the "Key rules" entry links to it. "Job Logging" introduces the `Job` as something a method gets by declaring a final `Job` parameter, and its example is a `CityPlaceSyncService` syncing one `City`'s `Place`s, on BearBnB nouns and with the `City.find` / return-early lookup background implementations take.
- **`SKILL.md`** — Critical Rule #20 makes its signature-defaults case on `quoteStayCents(place, { nights = 1, includeCleaningFee = true } = {})`, a plain exported function with no enqueue entry point. The advice is unchanged; the example is now a function that is never dispatched through `background`, where a defaulted trailing options bag is the one shape the appended `Job` would take over.
...
- **`controllers.md`** — a new "An absent key and an explicit `null` differ" subsection under Parameter Handling. With `{ allowNull: true }`, `castParam` returns `undefined` for a key absent from the body and `null` when the body carries an explicit `null` — the difference that lets a partial update tell "leave this column alone" apart from "clear it". The rule holds at every depth, including a `null` nested inside a present object. Both values are falsy, so test `=== undefined` wherever the two must stay apart, and collapse them freely where they need not — an `index` action's optional filters.

### Changed

- **`SKILL.md`** — ecosystem version baseline: `@rvoh/psychic` to 3.13.x (the other four packages unchanged).
- **`controllers.md`** — the "UPDATE with a nullable FK" worked example. It named `cityId` inside `extractParams`, which the `allowed` type forbids — a compile error at the call site, so the example could not be copied into a controller as written. It now pulls the foreign key with `castParam`, loads the `Place` through a visible `this.currentHost.associationQuery('places').findOrFail(...)` chain so the lookup and the authorization are one query, and passes the loaded `City` as an association, using a conditional spread to keep an absent key distinct from an explicit `null`. The lead-in now attributes the FK's nullability to the `cityId` column rather than to the `@deco.BelongsTo` `optional` flag, which the OpenAPI renderer never reads.
- **`controllers.md`** — the two `extractParams(Room, ['type', 'name'], { key: 'rooms', array: true })` examples, under "Other options" and in the nested multi-resource creation example. `'type'` is the STI discriminator, excluded unconditionally from a model's param-safe columns, so both allowlists were compile errors. They now extract `['name', 'sleeps']` and read the discriminator per item from the raw request body, matching `sti.md`; the nested example's `OpenAPI.forDream(Room, ...)` re-adds `type` to the spec with `including` and its transaction dispatches each item to the STI child class through an exhaustive switch.
- **`models.md`** — "Removing Default Scopes" opens by saying what the technique is for: reaching records a scope deliberately hides, when the operation is about those records, with [soft-delete.md](soft-delete.md) named for one such case. The section had taught which method suits which cardinality and never why you would reach for either. Two phrasings that read as promises are gone with it — the "Which to use" paragraph no longer closes "where you want to ensure the record is found regardless of any default scope", and the single-record example's comment says what the call does, lifting every filter so it returns whatever the table holds under that id, rather than calling it "safe".
...
- **`models.md`** — "Transforming a column on write" states that a `{ skipHooks: true }` query update cannot write an `@deco.Encrypted` column: naming the backing column throws unless the value is `null`. To update an encrypted column, use a path that instantiates the model — a plain query update, or `{ lock: true }` with or without `skipHooks` — and write through the encrypted property.
- **`locking.md`** — a new topic file owning guarded (compare-and-set) writes, registered in `SKILL.md` and in `README.md`'s topic table and rules-file reading list. It opens on what needs no lock: two requests writing different attributes of a record both land, two racing on the same attribute resolve to a single winner whether or not you lock, and uniqueness or interval-overlap invariants are constraint problems — a unique index, a PostgreSQL exclusion constraint — rather than locking problems. A lock earns its place only when the value being written depends on a value just read, claiming a record out of a state being the canonical case. Dream expresses that as a guarded write: records are claimed in batches, each batch opening its own transaction, re-selecting its rows `FOR UPDATE`, re-checking the query's conditions against the locked rows, then writing and committing — so a record another transaction has moved out of the query is left alone, and the return value is the count of records actually claimed.
- **`locking.md`** — `update(attributes, { lock: true })` and `update(callback, { lock })` each get a section with the case each is for. The attributes form is for a guard expressible in the query's `where` clause where every claimed record gets the same attributes; each claimed record is written through the ordinary per-instance update, so hooks, validations, and custom setters run as they would on `instance.update(...)`. `skipHooks: true` composes with `lock: true` and stays on that per-instance path — hooks skipped, custom setters (including the setters `@deco.Encrypted` attributes encrypt through) still run — and when the lifecycle should not run at all, the no-`lock` single-statement `update(attrs, { skipHooks: true })` is also a compare-and-set: its one `UPDATE ... WHERE` re-checks the conditions under each row's lock. The callback form is for a guard, or a new value, that has to be computed from the record itself — the only shape that can guard on a value no `where` clause can express, such as an `@deco.Encrypted` property, or a value derived from the old one. Under `lock: true` it runs once per claimed record, under that record's row lock inside the batch's transaction, and returns the attributes to write or `undefined` to skip that record; the worked example rotates an `@deco.Encrypted` door code, guarding on a plaintext value that is virtual and a ciphertext that is fresh on every write, so no `where` clause can express it. A callback requires an explicit boolean `lock` — omitting it throws `MissingRequiredLockOptionForUpdateCallback` — and `{ lock: false }` is the derived write without the guarantee: the callback runs per matching record through ordinary batched reads (`batchSize` 1000, no claim step, no compare-and-set), the shape for a derived backfill — reading from a possibly-lagging replica on a `@ReplicaSafe` model, where `.connection('primary')` pins the freshest committed state (`lock: true` reads the primary already, its reads being transactional). Only the attributes the callback returns are written, with assignments made onto the record it receives discarded, and the callback must not touch the database: the record it receives is not bound to the batch's transaction, so a read or write through it runs on a separate pooled connection and blocks on the batch's own locks, a cycle Postgres's deadlock detector cannot see.
- **`locking.md`** — `destroy({ lock: true })` (and `reallyDestroy`) as the same guarantee for removal, with destroy hooks and the `dependent: 'destroy'` cascade running per record and a `@SoftDelete()` model still soft-deleting; `lock` lives on the query and has no instance-level equivalent, since destroying an already-loaded instance has no select-then-destroy window to close.
- **`locking.md`** — a "What a lock costs" section stating the costs once: `batchSize` defaults to 10 under a lock rather than the 1000 an unlocked batch uses, because every row stays locked for the whole batch — each record's hooks and cascade, and, on the callback form, the query's `preload` queries; the locked read is a plain `FOR UPDATE` with no `NOWAIT` and no `SKIP LOCKED`, so a batch waits without bound on whoever holds the row, holding its transaction and connection open (set a `lock_timeout` where failing beats hanging); the guarantee is per batch, not set-wide, and threading `.txn(txn)` makes the run all-or-nothing without extending the compare-and-set across batches under READ COMMITTED, where a set-wide guarantee needs REPEATABLE READ or SERIALIZABLE — under which the same race raises a serialization failure instead of a quieter lower count; wrapping the call in `ApplicationModel.transaction(...)` without threading `.txn(txn)` hangs on the same invisible two-connection cycle; a `limit` or `offset` on the query throws `BatchingIncompatibleWithLimitOrOffset`; and an update without `lock` whose matched record is destroyed between a batch's read and its write throws, leaving earlier batches committed and returning no count. Writing a large set that does not need compare-and-set should pass no `lock` — it is slower than the plain form by construction.
- **`models.md`**, **`querying.md`** — a default (non-`skipHooks`) query update is described as issuing one `UPDATE` per matched row rather than as an N+1. It batches its reads through `findEach` and then writes per record, so the cost is a write per row and preloading does not address it. Both files name `{ skipHooks: true }` as what collapses that to one SQL statement, at the price of the callback lifecycle.
- **`querying.md`** — `skipHooks` is stated as the bulk path whose price is the lifecycle: it is the idiomatic way to write many rows in one SQL statement (there is no other, short of dropping to `toKysely()` for the same statement), and what it removes is the callback lifecycle — so before reaching for it, establish that the model's hooks carry no business logic that applies to *this* write. The safety judgment is per write, not per model: a hook guarding `status` transitions makes a bulk `status` write unsafe to skip, but says nothing about a bulk write to an unrelated column on the same model. When the hooks do apply, the fix is not to skip them: keep the default per-record update, or narrow the query.
- **`querying.md`** — the query-level update section names which query-level writers actually bypass the model: only the no-`lock` `update(attrs, { skipHooks: true })` and `delete()` do, while `destroy`, `reallyDestroy`, and `undestroy` instantiate each record even under `skipHooks`, so default scopes and the `dependent: 'destroy'` cascade still apply. Instantiation is not what makes a custom setter run there: soft delete and `undestroy` write through raw Kysely and `reallyDestroy` issues a `DELETE`.
...
- **`workers.md`** — the two-level fan-out pattern's expander job moves from `not_urgent` to `last` priority and its individual worker job from `default` to `not_urgent`, keeping the whole fan-out below `default` so a long bulk run can't hold up genuinely important application jobs at that tier; `last` is also what the Priority Levels section already reserves for check-in/heartbeat jobs, which the fan-out section now cross-references. A new key point states that the expander and individual-worker services must route to the same BullMQ queue — priority only orders jobs within one queue, and jobs on different queues never compete for the same worker slots, so splitting them across queues silently defeats the backpressure. A further key point covers isolating the fan-out to its own queue: under open-source BullMQ, use a native-mode `queue` rather than a named `workstream`, since a workstream's `group.priority` is silently ignored by OSS and would break the backpressure the same way; under BullMQ Pro, a named `workstream` works fine instead — Pro honors `group.priority`, and Pro is also the only way to rate-limit the individual jobs against an external dependency, which the section now points at the existing Rate Limiting (BullMQ Pro) section for.
...
- **`locking.md`**, **`querying.md`** — the worked "claim a booking exactly once" example is the locked form, `Booking.where({ id, status: 'pending' }).update({ status: 'confirmed' }, { lock: true })`, which runs the callback lifecycle on every claimed record: that is the claim idiom, and `{ skipHooks: true }` is not offered as a faster version of it. `querying.md`'s query-update section now points a claim — a write whose value depends on a value just read — at `{ lock: true }`, the compare-and-set that keeps the lifecycle, while stating that the single-statement `{ skipHooks: true }` form is also a compare-and-set whose choice is choosing to skip the hooks, and links to `locking.md`.
- **`querying.md`**, **`soft-delete.md`**, **`models.md`** — the guarded-write material lives in `locking.md`, and each file points there instead of carrying its own copy. `soft-delete.md` states that `destroy()` and `reallyDestroy()` accept `{ lock: true }` and that guarded removal is a concurrency concern rather than a soft-delete one; `models.md`'s query-update cost paragraph links to `locking.md` for guarding a read-then-write.
...
- **`querying.md`** — `.delete()` is described as what it is: a single `DELETE` statement that bypasses the model, so it writes no `deletedAt`, runs no `dependent: 'destroy'` cascade (database-level FK cascades still fire), and leaves the rows permanently gone even on a `@SoftDelete()` model.
...
- **`testing.md` Factory Pattern** — reused-enum columns in generated factories emit a TS-rejecting `'TODO'` literal plus a comment hint; `pnpm build:spec` fails fast at the factory rather than surfacing a runtime NOT NULL / enum-mismatch the first time the factory runs. Covers scalar and array forms.
