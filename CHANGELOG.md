...
- **`migrations.md`** — new "Raw Kysely Data Queries in Migrations" section: a migration's `db` handle carries the same `CamelCasePlugin` as every other Kysely instance Dream builds, so raw selects/updates/inserts are written in camelCase like everywhere else in a Dream app, even though the rest of a migration file (DDL, `DreamMigrationHelpers` calls) is snake_case; also notes Kysely silently drops `undefined` from `.set()`/`.values()`, turning a misspelled or genuinely-undefined field into a silent no-op write rather than an error. A second callout covers `.insertInto()`: a migration-local Kysely schema interface has no way to express a DB-level column default, so Kysely's types require supplying a column like `id` explicitly even when Postgres would generate it — pass the same raw SQL the default uses (e.g. `id: sql\`uuidv7()\``).
