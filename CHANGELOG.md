...
- **`models.md`** — "Transforming a column on write" states that a `{ skipHooks: true }` query update *without* `lock` can silently write plaintext into an `@deco.Encrypted` column: the `encrypted<Name>` guard is itself a custom setter, so with no instance built there is nothing to throw, and the raw column name type-checks because this form's attributes are typed as the raw table schema. The plaintext lands unencrypted with no error and later reads through the plaintext property fail or return garbage. Strong params can't reach it — encrypted columns aren't param-safe — so it takes hand-written code.
- **`models.md`** — the Dirty Tracking section documents `update(attrs, { lock: true })` as the concurrency-safe query-level update, with a `Booking` example claiming a pending row; it still runs hooks and custom setters, and its batching and locking caveats match the guarded-destroy form's.
- **`querying.md`** — the query-level update section names which query-level writers actually bypass the model: only `update(attrs, { skipHooks: true })` and `delete()` do, while `destroy` and `undestroy` instantiate each record even under `skipHooks`, so custom setters still run there. That makes the bulk update the one query path that can put plaintext into an `@deco.Encrypted` column, with a link to the `models.md` treatment.
