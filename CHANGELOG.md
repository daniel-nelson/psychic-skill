...
- **`models.md`** — "Transforming a column on write" states that a `{ skipHooks: true }` query update cannot write an `@deco.Encrypted` column: naming the backing column throws unless the value is `null`. To update an encrypted column, do not use `skipHooks`.
...
- **`locking.md`** — `update(attributes, { lock: true })` and `update(callback, { lock: true })` each get a section with the case each is for. The attributes form is for a guard expressible in the query's `where` clause where every claimed record gets the same attributes; each claimed record is written through the ordinary per-instance update, so hooks, validations, and custom setters run as they would on `instance.update(...)`. The callback form is for a guard, or a new value, that has to be computed from the record itself — the only shape that can guard on a value no `where` clause can express, such as an `@deco.Encrypted` property, or a value derived from the old one. It runs once per claimed record, under that record's row lock inside the batch's transaction, and returns the attributes to write or `undefined` to skip that record; the worked example rotates an `@deco.Encrypted` door code, guarding on a plaintext value that is virtual and a ciphertext that is fresh on every write, so no `where` clause can express it. `lock` is mandatory with a callback — omitting it throws `MissingRequiredLockOptionForUpdateCallback` — only the attributes the callback returns are written, with assignments made onto the record it receives discarded, and the callback must not touch the database: the record it receives is not bound to the batch's transaction, so a read or write through it runs on a separate pooled connection and blocks on the batch's own locks, a cycle Postgres's deadlock detector cannot see.
...
- **`querying.md`** — the query-level update section names which query-level writers actually bypass the model: only `update(attrs, { skipHooks: true })` and `delete()` do, while `destroy`, `reallyDestroy`, and `undestroy` instantiate each record even under `skipHooks`, so default scopes and the `dependent: 'destroy'` cascade still apply. Instantiation is not what makes a custom setter run there: soft delete and `undestroy` write through raw Kysely and `reallyDestroy` issues a `DELETE`.
...
- **`locking.md`**, **`querying.md`** — the worked "claim a booking exactly once" example is the locked form, `Booking.where({ id, status: 'pending' }).update({ status: 'confirmed' }, { lock: true })`, which runs the callback lifecycle on every claimed record: that is the claim idiom, and `{ skipHooks: true }` is not offered as a faster version of it. `querying.md`'s query-update section now names `{ lock: true }` as the tool whenever the value being written depends on a value just read, and links to `locking.md`.
