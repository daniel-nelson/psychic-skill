...
- **`models.md`** — "Transforming a column on write" states that a `{ skipHooks: true }` query update without `lock` cannot write an `@deco.Encrypted` column: naming the backing column throws unless the value is `null`, since nothing on that path encrypts it. Write through the plaintext property, or pass `lock: true`; `null` is how the single-statement path clears the column.
