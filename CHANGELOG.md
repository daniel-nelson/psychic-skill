...
- **`SKILL.md`** — Critical Rule 23: reach for the simplest shape that satisfies the requirement, and escalate only on evidence you can point at. Dream and Psychic expose real machinery for hard problems — row locks and compare-and-set writes, transactions, batching, replica routing, raw `sql` — and none of it is free, so complexity added in anticipation of a problem is a certain cost against a speculative benefit. Before choosing something heavier than a plain `update`, a `findEach`, or a database constraint, name the concrete scenario that breaks the simple shape; "a race is conceivable" is not one. A one-shot data correction is called out as the case that goes wrong most often — it is a `findEach` calling `update` on each affected row, or the `Query#update` callback form when the new value derives from the row — and a review finding that is an artifact of self-inflicted complexity is answered by removing the complexity, not by hardening it.
