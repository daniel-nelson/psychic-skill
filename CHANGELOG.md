...
- **`controllers.md`** — "Declaring `paramSafeColumns` on the model" now clarifies when the model-level guard is actually worth adding: a column is only ever extracted if some controller action's own allowlist names it, so `paramSafeColumns`/`paramUnsafeColumns` are for the narrower case of a specific column (an admin flag, a payout-account ID, a hand-managed tenancy column) that must never be assignable regardless of any call site's allowlist — not something to declare reflexively on every model.
