...
- **`models.md`** — "Transforming a column on write" states that a `{ skipHooks: true }` query update without `lock` cannot write an `@deco.Encrypted` column at all: naming the backing column throws unless the value is `null` (which is how that path clears it), and the plaintext property is virtual so the form does not accept it. Writing an encrypted column takes a form that instantiates the model — no options object, or `{ lock: true, skipHooks: true }`.
- **`querying.md`** — the query-level update section names which query-level writers actually bypass the model: only `update(attrs, { skipHooks: true })` and `delete()` do, while `destroy` and `undestroy` instantiate each record even under `skipHooks`, so custom setters still run there.
