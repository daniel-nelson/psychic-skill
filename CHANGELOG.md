...
- **`models.md`** — the Dirty Tracking section's closing paragraph is rewritten around the fact it was mis-stating: dirty tracking's baseline is the values the instance was *loaded* with, not the row's current values, so when another writer has changed the row, assigning the value the instance already holds marks nothing dirty and no `UPDATE` is issued — silently. `reload()` before assigning is what measures the assignment against what is actually stored. The query-update mechanics that shared that paragraph are gone; they were already in `querying.md`.
