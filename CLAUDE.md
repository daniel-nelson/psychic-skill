...
"Current state" is not the same as "published state." A change verified against real upstream Dream/Psychic source — even one still sitting on an unmerged branch in `~/work/dream_and_psychic` — is documented as current, matter-of-fact, the same as a shipped feature, once the maintainer has confirmed it. Do not hedge it as "coming soon," "in an upcoming release," or otherwise forward-looking, and do not wait for it to merge or publish before writing it up. This is a standing policy, not a one-off exception — it applies to every future triage pass that finds a verified-but-unmerged upstream change worth documenting.
