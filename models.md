...
A `{ skipHooks: true }` query update without `lock` — `Booking.where(…).update(attrs, { skipHooks: true })` — never instantiates a model, so no setter runs at all: the attribute names go into the `UPDATE` as given and the transform is silently skipped. That also lets it put plaintext into an `@deco.Encrypted` column: the `encrypted<Name>` guard is itself a custom setter, so with no instance there is nothing to throw, and the raw column name type-checks here because this form's attributes are typed as the raw table schema. The plaintext lands unencrypted with no error, and later reads through the plaintext property fail or return garbage. Strong params can't reach it — encrypted columns aren't param-safe — so this needs hand-written code. The instance form, `instance.update(attrs, { skipHooks: true })`, is unaffected — it assigns through `assignAttributes`, so custom setters still run; `skipHooks` there means hooks, not setters.
