...
**How stale is "stale"?** Actual lag depends on the managed Postgres provider. Aurora PostgreSQL replicas read from the same shared cluster storage volume as the writer, so lag is normally well under 100ms; AWS's own docs note it increases during heavy write periods without giving a specific ceiling. Plain RDS PostgreSQL replicas use WAL-based physical streaming replication to separate storage, so lag is normally sub-second but has more room to grow under heavy sustained write load, bounded by WAL generation rate, network throughput, and replica apply capacity rather than a fixed number. Both engines expose a `ReplicaLag` CloudWatch metric — check it for your actual workload rather than assuming a number. Either way, "read your own write" always means routing that specific read to primary via `.connection('primary')` — never something to paper over by assuming the replica will have caught up by the time the request reads it.
