...
A `{ skipHooks: true }` query update without `lock` — `Booking.where(…).update(attrs, { skipHooks: true })` — never instantiates a model, so no setter runs at all: the attribute names go into the `UPDATE` as given and the transform is silently skipped. That also makes it the one write path that can put plaintext into an `@deco.Encrypted` column: the `encrypted<Name>` guard is itself a custom setter, so with no instance there is nothing to throw, and the raw column name type-checks here because this form's attributes are typed as the raw table schema. The plaintext is written unencrypted with no error, and later reads through the plaintext property fail or return garbage. Strong params can't reach it — an encrypted column is excluded from the model's param-safe set — so this only arises in hand-written code. The instance form, `instance.update(attrs, { skipHooks: true })`, is unaffected — it assigns through `assignAttributes`, so custom setters still run; `skipHooks` there means hooks, not setters.
...
To force a write against the row's current database state regardless of the loaded instance's in-memory value (for example, resetting a field that another writer may have changed), use a query-level update (`Booking.where({ id }).update({ ... })`) or call `instance.reload()` *before* assigning the field(s) you're about to force-write — `reload()` overwrites every attribute on the instance from the database, so calling it after other unsaved changes on the same instance discards them too. The default (non-`skipHooks`) query-level `.update()` isn't a single bulk `UPDATE`: it loads and updates matching rows one at a time via `findEach`, running model hooks per row — fine when the `where` matches one row, as above, but an N+1 pattern if applied more broadly; pass `{ skipHooks: true }` for the actual single-statement bulk path. The default form is a read-then-write with an open race window, not a compare-and-set guarantee; the `skipHooks` form's single `UPDATE ... WHERE` *is* a compare-and-set claim when the `where` names the expected prior state. For a concurrency-safe update that still runs hooks and setters, pass `{ lock: true }`: `Booking.where({ status: 'pending' }).update({ status: 'confirmed' }, { lock: true })` claims each batch with `FOR UPDATE` inside its own transaction, writes every claimed record through the ordinary instance update — so hooks and custom setters run, and pairing `skipHooks: true` with `lock` skips only the hooks — and resolves to the number of records it claimed. Its batching and locking caveats match the destroy form's: see [soft-delete.md's "Guarded (compare-and-set) destroy"](soft-delete.md#guarded-compare-and-set-destroy).
