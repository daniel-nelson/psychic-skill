...
**How stale is "stale"?** Actual lag depends on the managed Postgres provider. Aurora PostgreSQL replicas read from the same shared cluster storage volume as the writer, so lag is normally sub-100ms and can reach up to roughly a minute under sustained heavy write load. Plain RDS PostgreSQL replicas use WAL-based physical streaming replication to separate storage, so lag is normally under a second but can spread into the tens of seconds under heavy write load (both engines expose a `ReplicaLag` CloudWatch metric to monitor this in production). Either way, "read your own write" always means routing that specific read to primary via `.connection('primary')` — never something to paper over by assuming the replica will have caught up by the time the request reads it.
