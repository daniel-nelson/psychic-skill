...
- **`background.queues` is empty until `background.connect()` runs** — a script boots the app without starting the server. See [workers.md — Inspecting Queues Outside a Booted Server](workers.md#inspecting-queues-outside-a-booted-server).
