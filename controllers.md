...
`paramSafeColumns` is the model-level upper bound on what `extractParams` can return and on what a model-derived `@OpenAPI` `requestBody`'s `params` can list — both derive from the same `paramSafeColumnsOrFallback()` set, so a column missing from the model's declaration is excluded from both, even if a controller's `params` names it. `including` is a separate mechanism and is **not** capped by `paramSafeColumns` — it re-adds a column straight from the model's real columns/virtual attributes regardless of param-safe status, which is exactly how it re-surfaces an FK or STI `type` in the spec after the always-excluded set drops it. That also means `including` can document a field in the spec that `extractParams` will still refuse to extract if it isn't param-safe — don't reach for `including` to work around a `paramSafeColumns`/`paramUnsafeColumns` restriction.
