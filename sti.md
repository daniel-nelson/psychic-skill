...
Dream allows changing an STI record's type after creation by updating the `type` column. Set every field the new type requires in the same update. Database-level check constraints (generated by `g:sti-child`) will reject invalid data regardless.
