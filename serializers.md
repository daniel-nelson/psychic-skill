...
**STI-aware preloading:** `preloadFor` on an STI base class discovers what every STI child's serializer renders, not just the base's. When different children's serializers render different `rendersOne`, `rendersMany`, or `delegatedAttribute` targets, `preloadFor` loads them all: the preload set is the union across every child's serializer, decided before the query knows which types it will return, so every row carries the whole union. This means a single `preloadFor` call on the base class correctly handles mixed result sets of different STI children.
