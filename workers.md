...
- **If the kickoff is interrupted,** only the expanders already enqueued will run, and if the kickoff job itself is retried it will re-pluck from the beginning — so every ID it had already reached is enqueued again, and `_processOne` redoes that record's work.
