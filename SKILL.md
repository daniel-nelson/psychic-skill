...
23. **Reach for the simplest shape that satisfies the requirement, and escalate only on evidence you can point at.** Dream and Psychic expose real machinery for hard problems — row locks and compare-and-set writes, transactions, batching, replica routing, raw `sql`. Every one of them exists for a situation most code never reaches, and none of it is free: a lock stalls writers that have nothing to do with your work, a validation layer grows its own failure modes, and a design a reviewer cannot hold in their head generates review findings whose only cause is the complexity itself. Complexity added in anticipation of a problem is a certain cost paid against a speculative benefit.

    The simple shapes are the ones this skill teaches first, and they are the answer far more often than they look: a plain `update`, a `findEach` over a query, a database constraint. Before choosing something heavier, name the concrete scenario that breaks the simple shape — a specific concurrent writer, a measured row count, an invariant the database cannot express. "A race is conceivable" and "this table might get large" are not that scenario.

    A one-shot data correction is the case that goes wrong most often. It is a `findEach` over the affected rows calling `update` on each, or the `Query#update` callback form when a row's new value has to be derived from the row itself. It is not a table lock, not a digest-verified manifest, and not a re-validation of the whole table's population on every run. A correction script that reads like a distributed-systems protocol is over-engineered, not careful.

    **When a review finding is an artifact of complexity you introduced, remove the complexity instead of hardening it.** Bounding the wait on a lock you never needed to take fixes the symptom and keeps the cost.
